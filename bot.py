        ...
        try:
            ...
            with db() as conn:
                conn.execute(
                    ...
                    "ON CONFLICT(user_id) DO UPDATE SET phone=excluded.phone,session_name=excluded.session_name,logged_in=0",
                    (uid, phone, session_name(uid)),
                )
                conn.commit()
            clients[uid] = client
            states[uid] = {"step": "code", "phone": phone}
            await message.answer("📩 Kode Telegram sudah dikirim. Kirim kodenya di sini.")
