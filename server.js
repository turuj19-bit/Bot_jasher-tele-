...
/*
  FOTO BANNER START
  -----------------
  Prioritas: START_BANNER_FILE_ID (lebih stabil karena file sudah ada di Telegram).
  Fallback: START_BANNER_URL (harus direct image URL yang bisa diambil Telegram).
*/
...
async function downloadBotPhoto(
  ...
  if (!buffer.length) {
    throw new Error(
      "File foto Telegram kosong."
    );
  }

  /*
    GramJS menentukan foto/document dari nama file.
    Photo Telegram biasanya berupa JPEG.
  */
  ...
  */
  ...
  if (c.media_file_id) {
    try {
      ...
      try {
        await bot.api.sendMessage(
          u.telegram_user_id,
          `❌ Foto format gagal diambil dari Telegram.\n\n${escapeHtml(reason.slice(0, 500))}`,
          {
            parse_mode: "HTML"
          }
        );
  ...
  for (
    ...
    try {
      ...
      /*
        Pesan SUDAH berhasil terkirim.
        Jadi langsung hitung berhasil.
      */
      berhasil++;

      /*
        Kalau pencatatan log gagal, jangan ubah
        status pengiriman menjadi gagal.
      */
      try {
        const {
          error:
      ...
      /*
        Error log hanya untuk pencatatan.
        Kalau insert log gagal, tidak mengubah
        hasil pengiriman.
      */
      try {
        const {
  ...
  try {
    let report =
      ...
      `Total group dipilih : <b>${totalDipilih}</b>\n` +
      `Berhasil terkirim   : <b>${berhasil}</b>\n` +
      `Gagal               : <b>${gagal}</b>`;
